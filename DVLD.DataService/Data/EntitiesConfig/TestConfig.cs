...
internal class TestConfig : IEntityTypeConfiguration<Test>
{
    public void Configure(EntityTypeBuilder<Test> builder)
    {
        ...
        builder.HasOne(x => x.User).WithMany(x => x.TestsCreated).HasForeignKey(x => x.CreatedByUserId)
            .OnDelete(DeleteBehavior.NoAction);
        builder.HasOne(x => x.TestAppointment).WithOne(x => x.Test)
            .HasForeignKey<Test>(x => x.TestAppointmentId).OnDelete(DeleteBehavior.NoAction);
