...
internal class DriverConfig : IEntityTypeConfiguration<Driver>
{
    public void Configure(EntityTypeBuilder<Driver> builder)
    {
        ...
        builder.HasOne(x => x.User).WithMany(x => x.DriversCreated)
            .HasForeignKey(x => x.CreatedByUserId).OnDelete(DeleteBehavior.NoAction);
        builder.HasOne(x => x.Person).WithOne(x => x.Driver)
            .HasForeignKey<Driver>(x => x.PersonId).OnDelete(DeleteBehavior.NoAction);
    }
}
