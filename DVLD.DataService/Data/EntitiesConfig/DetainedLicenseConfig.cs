...
internal class DetainedLicenseConfig : IEntityTypeConfiguration<DetainedLicense>
{
    public void Configure(EntityTypeBuilder<DetainedLicense> builder)
    {
        ...
        builder.HasOne(x => x.License).WithMany(x => x.DetainedLicense)
            .HasForeignKey(x => x.LicenseId).OnDelete(DeleteBehavior.NoAction);
        ...
        builder.HasOne(x => x.Application).WithMany(x => x.DetainedLicense)
